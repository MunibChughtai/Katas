...
class Bj_game:
    ...
    def prepare_for_game(self):
        self._deck.deck_setup() # input cards into the deck
        self._player._card_set.get_first_2_cards(self._deck) # pick first 2 card by player
        self._dealer._card_set.get_first_2_cards(self._deck) # pick first 2 card by dealer

    def start_game(self, test_mode_no_input_needed=0):
        ...
        if(self._gotoconsole_player(test_mode_no_input_needed) == 'Bust!'):
            return ('Dealer wins!')
            #return
    ...
    def _gotoconsole_player(self, test_mode_no_input_needed):
        while True:
            ...
            user_input = 0 if test_mode_no_input_needed else self._get_user_input()
            if int(user_input):
